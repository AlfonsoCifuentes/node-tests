...
const MongoStore = require('connect-mongo') //requiere instalación de connect-mongo

const { DB_URL } = require('./db/db'); //Almacenamos con destructuring la const con la url de nuestra db
...
server.use(session({
    ...
    store: MongoStore.create({ mongoUrl: DB_URL }) //Con esto hacemos el mantenimiento de sesión
}));

// Middleware para la autenticación (registro/login)
server.use(passport.initialize());

// Middleware para sesión de auth -- requiere instalar express-session
